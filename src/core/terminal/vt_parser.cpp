...
void VtParser::emitEvent(const Event& ev)
{
    if (ev.type == Event::CsiDispatch) {
        ...
        spdlog::debug("[VT-CSI] final='{}' private={} params={}", ev.csiFinal, ev.csiPrivate, params);
    } else if (ev.type == Event::Print) {
        if (ev.ch < 0x20 || ev.ch == 0x7F) {
            spdlog::debug("[VT-PRINT] ch=<{:02X}> width={}", ev.ch, ev.width);
        } else if (ev.ch == '[' || ev.ch == ']' || ev.ch == 'm' || ev.ch == 'H') {
            spdlog::debug("[VT-PRINT] ch='{}' width={}", static_cast<char>(ev.ch), ev.width);
        }
    } else if (ev.type == Event::Execute) {
        spdlog::debug("[VT-EXEC] ctrl=<{:02X}>", ev.ctrl);
    } else if (ev.type == Event::OscDispatch) {
        spdlog::debug("[VT-OSC] cmd={} data_len={}", ev.oscCommand, ev.oscData.size());
    } else if (ev.type == Event::EscDispatch) {
        spdlog::debug("[VT-ESC] final='{}'", ev.escFinal);
    }
...
void VtParser::oscCollect(uint8_t byte)
{
    ...
    // 解析命令号
    if (oscCommand_ == -1 && byte == ';') {
        // 找到分号, 之前的是命令号
        size_t semicolonPos = oscBuffer_.find(';');
        if (semicolonPos != std::string::npos) {
            try {
                oscCommand_ = std::stoi(oscBuffer_.substr(0, semicolonPos));
            } catch (...) {
                oscCommand_ = 0;
            }
        }
...
int VtParser::charWidth(uint32_t ch)
{
    ...
    // CJK 统一汉字扩展 B-I (需要代理对, 但 UTF-32 直接表示)
